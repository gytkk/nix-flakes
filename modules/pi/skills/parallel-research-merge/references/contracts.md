...
Use 2 scopes by default. Use 3 only when the task clearly has three disjoint concerns. Run them as parallel workers only when the current Pi session exposes a worker or subagent tool; otherwise, execute the scopes sequentially in the main thread.
...
Task: add a new Pi behavior rule without bloating always-on guidance.

- Scope A: inspect current Pi skill layout and repo-managed deployment path
