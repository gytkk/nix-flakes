...
- Follow Pi's Agent Skills format when creating or updating skills for this
  setup: each skill uses a focused `SKILL.md` with valid `name` and
  `description` frontmatter.
- Store repo-managed Pi skills under `modules/pi/skills/` so Home Manager
  exposes them at `~/.pi/agent/skills/`.
- Keep skills focused, trim unused files, and align instructions with this
  repository's conventions before committing.
- Use the repo-managed `parallel-research-merge` skill when a task needs
  bounded multi-axis investigation before one main agent produces the final
  implementation.
- Pi does not provide subagents by default. Use parallel workers only when the
  current session exposes a worker or subagent tool; otherwise, execute the
  same bounded research scopes sequentially.
