...
let
  ...
  guardrailsConfigPath = "/etc/openclaw/openclaw.guardrails.json";
  ...
  guardrailsConfig = {
    gateway = {
      mode = "local";
      auth = seedConfig.gateway.auth;
      port = gatewayPort;
      bind = "loopback";
      trustedProxies = seedConfig.gateway.trustedProxies;
      http = seedConfig.gateway.http;
      tailscale.mode = "off";
      controlUi = seedConfig.gateway.controlUi;
    };
...
in
{
  ...
  environment.etc."openclaw/openclaw.guardrails.json".text = builtins.toJSON guardrailsConfig;
  ...
  services.openclaw-gateway = {
    ...
    execStartPre = [
      ...
      "${pkgs.writeShellScript "openclaw-runtime-config" ''
        ...
        guardrails_config=${pkgs.lib.escapeShellArg guardrailsConfigPath}
        ...
        ${pkgs.jq}/bin/jq -e . "$guardrails_config" >/dev/null

        merged_file="$(${pkgs.coreutils}/bin/mktemp "$state_dir/openclaw.json.merge.XXXXXX")"
        ${pkgs.jq}/bin/jq -s '.[0] * .[1]' "$runtime_config" "$guardrails_config" > "$merged_file"
        ${pkgs.coreutils}/bin/chmod 600 "$merged_file"
        ${pkgs.coreutils}/bin/mv -f "$merged_file" "$runtime_config"
      ''}"
    ];
