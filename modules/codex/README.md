...
Herdr 안에서 실행한 Codex는 Agents 항목 아래에 직계 subagent의 상태, 작업 이름, 실제 모델, 최근 활동을 두 줄씩 표시한다. 첫 줄의 상태와 이름·모델을 분리하고 둘째 줄의 활동 문구는 옅게 표시한다. 이름은 Codex의 `agent_path`에서 가져오며, 모델은 자식 세션의 `turn_context`를 사용한다. 부모 행에는 실행 중인 자식 수를 표시한다. 별도의 접기 버튼이나 terminal pane은 만들지 않는다. 표시 개수와 레이아웃은 [Herdr README](../herdr/README.md#subagents)에서 확인한다.

상태는 Codex의 작업 이벤트로 결정한다. 초록색 `● 실행 중`, 파란색 `✓ 완료`, 노란색 `■ 중단`, 빨간색 `× 실패`를 구분하며, 완료·중단·실패는 이벤트 발생 후 5초 동안 표시한다. 같은 subagent에 후속 작업을 보내면 다시 실행 중으로 바뀐다. 명령 실행 완료나 메시지 갱신은 활동 문구만 바꾸며 작업 상태를 변경하지 않는다. 출력이 없는 동안에도 종료 이벤트를 받기 전에는 실행 중 상태를 유지한다.
...
실행 중인 자식의 기록을 읽지 못하거나 기록이 손상되면 회색 `○ 상태 확인 불가`와 마지막 활동을 표시하며, 확인된 실행 수에서 제외한다. 기록을 다시 읽어 작업 상태를 확인하면 정상 표시로 돌아간다. 감시 프로세스 자체가 종료되거나 Herdr에 전송할 수 없으면 새로운 상태를 표시할 수 없으므로 마지막 metadata가 15초 뒤 만료된다.

활동 문구는 assistant 메시지나 도구 종류를 사용한다. 작업 시작 직후에는 `작업 중`을 표시하며, reasoning, 암호화된 메시지, 원시 명령어는 표시하지 않는다. 이 연동은 Codex CLI 0.155.1과 Herdr 0.9.1을 기준으로 검증했다. Codex의 내부 JSONL 형식은 안정된 API가 아니므로 CLI 업데이트 후 표시가 달라지면 실제 기록 형식과 parser를 확인한다.
