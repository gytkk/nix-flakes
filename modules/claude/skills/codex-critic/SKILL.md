...
**인증**: `OPENAI_API_KEY` 환경변수 또는 `codex login`을 통한 ChatGPT 인증 중
하나가 설정되어 있으면 된다. 인증 실패는 codex exec 실행 시 자체적으로 보고된다.
...
| 환경변수                | 기본값    | 설명                                             |
| ----------------------- | --------- | ------------------------------------------------ |
| `OPENAI_API_KEY`        | (선택)    | OpenAI API 키. 미설정 시 `codex login` 인증 사용 |
