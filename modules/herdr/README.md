...
Codex shows a separate colored status marker and label beside each child's name and model, with dimmed activity below it and a running count on the parent row. Running, completed, interrupted, and failed are separate states. Terminal states remain visible for five seconds. Read failures show an unknown state without counting the child as confirmed running. If the watcher dies or cannot reach Herdr, its last metadata expires within 15 seconds; expiration removes rows rather than generating an unknown label.
