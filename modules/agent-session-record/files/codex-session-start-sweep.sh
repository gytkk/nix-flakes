...
warn() {
  ...
  printf '%s codex-session-start-sweep: %s\n' \
...
payload_file=""
if ! payload_file="$("$MKTEMP" "${TMPDIR:-/tmp}/codex-session-start-sweep.XXXXXX")"; then
  warn "mktemp failed"
  continue_json
  exit 0
fi
...
if ! cat >"$payload_file"; then
  warn "failed to persist SessionStart payload"
