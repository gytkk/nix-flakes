...
if [ ! -d "$CLAUDE_PROJECTS_DIR" ]; then
  "$PRINTF" 'claude projects directory missing: %s\n' "$CLAUDE_PROJECTS_DIR" >&2
  exit 1
fi
...
remote_claude_dir="${AGENT_SESSION_RECORD_REMOTE_BASE_PATH}/claude"
...
cleanup() {
  ...
  if [ -n "$transcripts_file" ] && [ -f "$transcripts_file" ]; then
    "$RM" -f "$transcripts_file"
  fi
}

trap cleanup EXIT

transcripts_file="$("$MKTEMP" "${TMP_DIR}/claude-upload-transcripts.XXXXXX")"
"$FIND" "$CLAUDE_PROJECTS_DIR" -type f -name '*.jsonl' ! -path '*/subagents/*' -print | "$SORT" >"$transcripts_file"

total_transcripts="$("$WC" -l <"$transcripts_file" | "$TR" -d ' ')"
if [ "${total_transcripts:-0}" -eq 0 ]; then
  "$PRINTF" 'No Claude session transcripts found under %s\n' "$CLAUDE_PROJECTS_DIR"
  exit 0
fi
...
while IFS= read -r transcript_path <&3; do
  ...
  payload_file="$("$MKTEMP" "${TMP_DIR}/claude-upload-payload.XXXXXX")"
  "$JQ" -n \
  ...
  if "${BASH:-bash}" "$WORKER_CMD" --mode payload --agent claude --payload-file "$payload_file" </dev/null; then
    if [ -f "${SESSION_STATE_DIR}/claude-${session_id}.json" ]; then
      "$PRINTF" 'uploaded: %s\n' "$session_id"
  ...
  "$RM" -f "$payload_file"
  payload_file=""
done 3<"$transcripts_file"

remote_count="$("$SSH" -o BatchMode=yes -o ConnectTimeout=10 "$remote_prefix" \
  "find '$remote_claude_dir' -type f -name '*.jsonl' 2>/dev/null | wc -l" </dev/null || true)"

"$PRINTF" '\nprocessed: %s\nuploaded: %s\nfailed: %s\n' \
  "$total_transcripts" "$uploaded_count" "$failed_count"
