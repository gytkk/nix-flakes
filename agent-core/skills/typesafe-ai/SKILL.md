...
This installation uses the official TypeSafe skill with a local agenix integration. Run API clients through `with-jev COMMAND [ARG...]`, for example `with-jev uv run app.py` or `with-jev node app.mjs`. The wrapper supplies `TYPESAFE_API_KEY` from the managed secret only to that process and its children. Read the key from the environment inside the client; keep its value out of command arguments, output, and generated files. If the wrapper reports an unreadable secret, the environment's agenix activation needs attention.

Use the direct TypeSafe API and the existing project's SDK or HTTP client. This skill does not install an SDK. The upstream instructions below apply to workflow design and API usage.

<!-- Upstream: typesafe-ai/skills, commit 65a39f393687675ce170e6094757de20370365b9, skills/typesafe-ai/SKILL.md. Only the local API access section is added. -->
