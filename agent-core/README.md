...
모든 repository-managed skill은 portable skill로 관리한다. Portable은 skill source가 특정 실행 runtime에 묶이지 않는다는 뜻이다. 모든 runtime에 반드시 노출해야 한다는 뜻은 아니며, runtime별 노출은 `manifest.toml`의 allowlist가 결정한다. 현재 manifest는 14개 skill을 네 runtime에 모두 노출한다.

새 skill을 추가할 때 runtime별 복사본을 만들지 않는다. 특정 tool을 사용할 수 있을 때 더 빠른 흐름을 제공하는 skill은 capability를 먼저 확인하고, capability가 없을 때 실행할 순차 경로를 설명해야 한다.
...
Runtime module은 공통 원문, portable skill 원문, 독자적인 render 또는 merge 알고리즘을 소유하지 않는다.
...
- `render`는 manifest와 canonical resource를 읽고 runtime output tree를 생성한다. Output 경로는 없거나 비어 있는 실제 디렉터리여야 한다.
...
파일 쓰기와 CLI parsing은 이 API 밖에 둔다. 같은 source와 manifest는 byte 단위로 같은 결과를 만들어야 한다.
...
`CLAUDE.md`는 공통 operating rule, 공통 agent rule, prose rule, Claude adapter 순서로 생성한다. Home Manager는 문서와 skill tree를 `~/.claude/`에 설치한다. Local Claude marketplace에는 LSP plugin만 남기고 portable skill plugin은 두지 않는다.
...
`AGENTS.md`는 공통 operating rule, 공통 agent rule, prose rule, Codex adapter 순서로 생성한다. NixOS는 skill tree를 `/etc/codex/skills`에 설치한다. Standalone Home Manager 환경도 같은 render output을 system 경로에 연결한다.
...
Pi는 operating invariant를 `APPEND_SYSTEM.md`로 분리한다. `AGENTS.md`에는 공통 agent rule, prose rule, Pi adapter만 포함한다. 이 분리는 Pi가 operating invariant를 system prompt에 추가하면서 `AGENTS.md`에서 같은 내용을 다시 읽지 않게 한다.
...
`agent-core/nix/package.nix`는 wheel에 manifest와 canonical resource를 포함한 Python application을 만든다. `agent-core/nix/render.nix`는 runtime 이름만 받아 build derivation 안에서 renderer를 실행한다.
...
Runtime module은 `render.nix`가 만든 immutable tree에서 필요한 파일을 설치한다. Nix evaluation과 renderer는 홈 디렉터리, checkout 밖의 mutable state, runtime state를 읽지 않는다.
...
- Runtime별 skill 복사본과 독자적인 merge logic이 없다.
