#!/usr/bin/env python3
"""Restricted stdin-only release deployment command for the Astro blog."""
...
def main() -> int:
    ...
    try:
        ...
        print(f"astro-blog deployment rejected: {error}", file=sys.stderr)
        return 1
    except (OSError, tarfile.TarError, EOFError, ValueError, TypeError):
        print(
            "astro-blog deployment rejected: deployment operation failed",
            file=sys.stderr,
        )
        return 1
    return 0
