...
- warning log 기록 자체도 best-effort다.
  `mkdir -p`, `printf >> warnings.log` 모두 `|| true`로 감싸고, warning 기록 실패가
  추가 실패를 만들면 안 된다.
...
rsync -az "$local_file" \
  "gytkk@pylv-onyx:/home/gytkk/agent-sessions/claude/$host/$ym/$name.$fingerprint.tmp"
ssh gytkk@pylv-onyx \
  "mv /home/gytkk/agent-sessions/claude/$host/$ym/$name.$fingerprint.tmp \
