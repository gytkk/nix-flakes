...
- 업로드 hook 실패는 warning log만 남기고, Claude 종료나 Codex continue를
  막지 않음
...
사용자 세션의 종료/계속 흐름은 절대 막지 않고, 로컬 warning log만 남긴다.
...
- 원격 상대 경로 계산:
  `agent/<hostname>/<YYYY-MM>/<session_id>.(jsonl|meta.json)`
- 실패/skip 사유를
...
```bash
ssh gytkk@pylv-onyx "mkdir -p /home/gytkk/agent-sessions/claude/$host/$ym"
rsync -az "$local_file" \
  "gytkk@pylv-onyx:/home/gytkk/agent-sessions/claude/$host/$ym/$name.tmp"
ssh gytkk@pylv-onyx \
  "mv /home/gytkk/agent-sessions/claude/$host/$ym/$name.tmp \
      /home/gytkk/agent-sessions/claude/$host/$ym/$name"
```
...
- worker spawn 또는 사전 검증이 실패해도 warning log만 남기고 exit `0`
...
- worker spawn 또는 sweep 초기화 실패 시에도 warning log만 남기고
  `{"continue":true}` 유지
...
- **SessionEnd / Stop hook 내부 실패**: warning log만 남기고 Claude 종료 / Codex
  continue는 그대로 진행
...
6. **로컬 통합**: `pylv-onyx`에서 local short-circuit로
   `/home/gytkk/agent-sessions/{claude,codex}` 생성 및 파일 도착 확인
7. **원격 Claude 통합**: `pylv-denim`에서 Claude 세션 종료 후 onyx 도착 확인
8. **원격 Codex 통합**: `pylv-denim`에서 Codex 대화 2턴 진행 후 같은
   `session_id`가 최신 snapshot으로 덮어써지는지 확인
9. **Codex 재기동 복구**: 업로드 실패 상태를 만든 뒤 Codex 재시작 시
   `SessionStart` sweep이 queue/stale transcript를 비우는지 확인
10. **오프라인 재시도**: onyx 중단 상태에서 transcript 생성 후 queue 적재 확인,
   복구 뒤 다음 hook에서 업로드되는지 확인
11. **비정상 종료**: Claude는 Ctrl+C / SIGTERM, Codex는 강제 종료 후 다음
   `SessionStart`에서 복구되는지 확인
12. **hook 지연**: 세션 종료 체감 시간이 늘어나지 않는지 확인
