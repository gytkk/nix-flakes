...
따라서 중앙 저장 자체는 두 에이전트 모두 가능하지만, 트리거 전략은 다르게
잡아야 한다.

## 목표

- 모든 머신의 Claude Code 세션 종료 시 transcript를 `pylv-sepia`로 자동 전송
- 모든 머신의 Codex 세션 transcript도 같은 중앙 저장소로 자동 전송
- Ctrl+C / SIGTERM / 일시적 네트워크 실패에도 데이터 유실 없이 재시도
- `pylv-sepia` 다운 시에도 로컬 queue 또는 다음 기동 시 sweep으로 복구
...
└─────────────────┘
                        ...
                        ▼                          ▼                         ▼
                ┌──────────────┐       ┌────────────────────┐      ┌──────────────┐
                │ copyparty    │       │ local queue/state │      │ sepia 로컬   │
                │ on sepia     │◄──────┤ XDG_STATE_HOME 기반  │    │ cp (단축)    │
                │ (Tailscale)  │       │ retry + sweep index  │    └──────────────┘
                └──────────────┘       └──────────────────────┘
```

## 저장 레이아웃

`pylv-sepia` 상의 copyparty 볼륨 구조:
...
- 대상 URL 결정: `sepia`에서 실행 중이면 로컬 `cp`, 아니면 copyparty PUT
- 업로드 실패 시 `${XDG_STATE_HOME:-$HOME/.local/state}/agent-session-record/`
  아래 queue에 적재
- queue 재시도는 `flock`으로 직렬화
...
nohup "$HOME/.local/bin/agent-session-upload-worker" \
  --agent claude \
...
Codex는 공식 문서 기준으로 `Stop` hook이 **plain text stdout을 허용하지
않고**, exit `0`일 때 JSON을 기대한다. 따라서 Claude wrapper와 달리
background worker를 띄운 뒤에도 **반드시 유효한 JSON을 stdout에 출력**해야
한다.
...
- stdout에는 Codex hook 규격에 맞는 성공 JSON 또는 빈 additional context만 반환
...
→ `modules/claude/files/settings.json`를 갱신하되, 기존 hook 구조와 충돌하지
않도록 merge 필요
...
`modules/codex/files/config.toml` 또는 별도 option merge로
`features.codex_hooks = true`를 관리
...
options.modules.agentSessionRecord = {
  ...
  endpoint = lib.mkOption {
    type = lib.types.str;
    default = "http://sepia:3923/agent-sessions";
    description = "copyparty endpoint for agent session uploads";
  };
  localShortCircuit = lib.mkOption {
    type = lib.types.bool;
    default = false;
    description = "If true, copy to local path instead of HTTP";
  };
...
- `modules/claude/default.nix`
  - Claude hook command를 settings.json에 연결
- `modules/codex/default.nix`
  - `hooks.json` 배포
  - `features.codex_hooks = true` 활성화
  - 현재 `history.persistence = "save-all"` 유지

`base/pylv/sepia.nix`에서만 `localShortCircuit = true`로 override한다.

### 6. Copyparty 측 수신 볼륨

`pylv-sepia` copyparty 설정에 전용 볼륨 추가:

- 경로: `/var/lib/copyparty/agent-sessions/`
- 내부 레이아웃: `claude/`, `codex/`
- 권한: write-only upload + sepia 로컬 read
- 네트워크: Tailscale 대역 우선
- 인증: 1차는 내부망 전제, 필요 시 agenix 토큰으로 강화

현재 실제 구성은 root 볼륨(`/srv/copyparty`)에 익명 read/write가 열려 있으므로,
이 작업을 구현할 때는 세션 전용 경로를 별도로 분리하는 것이 좋다.

### 7. 인증 토큰 관리

옵션 A — Tailscale 내부망 무인증:

- 구현이 단순
- 현재 sepia 접근 모델과 잘 맞음
- 같은 tailnet 내부 장치에는 열림

옵션 B — agenix 암호화 토큰:

- `secrets/agent-session-upload-token.age` 추가
- 업로드 스크립트에서 header 또는 query token으로 사용
- copyparty 측 검증 규칙 추가

**우선 옵션 A로 시작하고, 중앙 저장이 안정화되면 옵션 B로 강화**한다.

## 엣지 케이스

- **sepia 자체에서 실행**: HTTP 대신 로컬 `cp` 사용
...
3. **Codex fallback unit**: `transcript_path = null`일 때
   `~/.codex/sessions/**/rollout-*-${session_id}.jsonl` 탐색이 맞는지 확인
4. **로컬 통합**: `localShortCircuit = true`로 `pylv-sepia`에서 Claude/Codex
   모두 파일 생성 확인
5. **원격 Claude 통합**: `pylv-denim`에서 Claude 세션 종료 후 sepia 도착 확인
6. **원격 Codex 통합**: `pylv-denim`에서 Codex 대화 2턴 진행 후 같은
   `session_id`가 최신 snapshot으로 덮어써지는지 확인
7. **Codex 재기동 복구**: 업로드 실패 상태를 만든 뒤 Codex 재시작 시
   `SessionStart` sweep이 queue/stale transcript를 비우는지 확인
8. **오프라인 재시도**: sepia 중단 상태에서 transcript 생성 후 queue 적재 확인,
   복구 뒤 다음 hook에서 업로드되는지 확인
9. **비정상 종료**: Claude는 Ctrl+C / SIGTERM, Codex는 강제 종료 후 다음
   `SessionStart`에서 복구되는지 확인
10. **hook 지연**: 체감 종료 시간이 늘어나지 않는지 확인

## 롤아웃 순서

1. `pylv-sepia` copyparty에 `agent-sessions` 볼륨 추가
2. `modules/agent-session-record/` 공통 worker 및 wrapper 작성
3. `modules/claude/default.nix`에 `SessionEnd` hook 연결
4. `modules/codex/default.nix`에 `hooks.json` 및 `codex_hooks` 활성화 추가
5. `pylv-denim`에서 Claude + Codex 둘 다 엔드-투-엔드 검증
6. 나머지 환경(`devsisters-*`, `pylv-onyx`)으로 확장
7. `pylv-sepia` 자체에서도 local short-circuit로 활성화
...
- `modules/claude/files/settings.json` hook merge를 어떤 방식으로 안전하게 할지
- `modules/codex/files/config.toml`에서 feature flag를 정적으로 둘지 option으로
  merge할지
- `pylv-sepia`의 copyparty 세션 전용 볼륨을 어디에 어떻게 분리할지
