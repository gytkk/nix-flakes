...
| 해결 상태 | 아래 해결안은 제안이며, 이번 작업은 리뷰 문서 작성까지다. |
...
| [4](#issue-4) | 두 갈래의 로컬 패키지 등록 경로 | minor | 등록과 CI 경로의 차이 확인 | 4 |
...
| [8](#issue-8) | 현재 구조와 어긋난 안내 문서 | minor | 삭제된 경로와 오래된 동작 설명 확인 | 각 변경과 함께 |
...
- [ ] 기존 모듈의 패키지 참조와 별칭이 그대로 동작한다.
- [ ] 이동만으로 버전, source hash, 설치되는 실행 파일이 달라지지 않는다.
- [ ] 세 패키지의 변경도 CI에서 감지한다.
- [ ] 자동 업데이트 도입 여부를 디렉터리 이동과 별개로 검토한다.
...
- [ ] 현행 안내 문서가 존재하지 않는 모듈을 관리 중인 앱으로 설명하지 않는다.
- [ ] 설정 원본과 적용 방식이 실제 코드와 일치한다.
- [ ] 같은 앱 동작을 여러 문서에서 독립적으로 수정할 필요가 줄어든다.
- [ ] 관련 동작을 바꾸는 변경에 해당 문서 수정도 포함된다.
...
| 로컬 패키지 추가 | `packages/apps/` | 별도 등록되는 예외 패키지는 이슈 4에서 정리한다. |
...
4. 이슈 4, 5, 6은 서로 다른 파일 범위에서 작은 변경 단위로 진행한다. 관련 검증을 먼저 연결하거나 같은 변경에 포함한다.
5. 이슈 7의 공통 사본을 정리하고, 이슈 8의 현행 문서는 각 변경과 함께 갱신한다.
