...
plugins=(git history zsh-autosuggestions z asdf)
...
sign () {
    ...
    vault write -field=signed_key ssh-client-signer/sign/developer valid_principals=ubuntu,ec2-user,core,openvpnas,admin,devadmin,devsisters-spark public_key=- ttl=${1-'86400'} < "$PUBKEY_PATH" > "$CERT_PATH"
    chmod 600 "$CERT_PATH"
}
