...
- `modules/herdr/files/plannotator-tui.toml` configures four `#RRGGBB` colors under `[theme]`: `selection_background` for selected paragraphs and file rows, `comment_background` and `looks_good_background` for document annotation highlights, and `markdown_code_foreground` for inline and fenced code. The checked-in values suit a light terminal theme; omitted settings preserve upstream colors. Interactive reviews load colors at startup and retain them across file and message navigation; headless commands retain upstream colors. Apply Home Manager to install the matching reviewer before opening reviews with newly added color settings.
