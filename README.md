...
- Home Manager registers the full [Herdr Annotate plugin](https://github.com/plannotator/herdr-annotate) from `packages/apps/herdr-annotate` in the Nix store. Its plugin source, annotation binary, and document review source and Cargo dependencies are pinned by hash. Nix builds the document reviewer with a local patch that exposes `[theme].selection_background` as a `#RRGGBB` color in `modules/herdr/files/plannotator-tui.toml`. Selected paragraphs and file rows use One Half Light's light gray (`#e5e5e5`); omitting the setting preserves the upstream default. Apply Home Manager before using this setting with an older, unpatched reviewer. The existing `Update App Versions` workflow checks for updates every three hours; pull those commits and apply Home Manager to install them. Reapplying an older Home Manager generation registers that generation's plugin version while preserving annotation data.
