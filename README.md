...
- `Ctrl+Shift+Tab` or `Ctrl+Tab` focuses the previous or next tab. In Ghostty on macOS, `Cmd+[` or `Cmd+]` focuses the previous or next workspace, `Cmd+Shift+[` or `Cmd+Shift+]` focuses the previous or next agent in sidebar order, `Cmd+t` creates a Herdr tab, and `Cmd+w` closes the current Herdr tab. `Cmd+1..9` focuses a workspace by index in Ghostty on macOS, and `Alt+1..9` does the same in Windows Terminal on WSL. `Ctrl+Space`, then `Shift+1..9` focuses the corresponding visible agent. The terminal configs forward direct shortcuts as distinct Kitty keyboard sequences so Herdr receives every modifier.
...
- Agents entries show the agent name and reported model on their second row. Pi and Codex also display subagent names, models, and activity. Codex shows lifecycle status markers separated from child names by one space and excerpts of recent assistant messages, including direct subagents working in the background. See [subagents](modules/herdr/README.md#subagents) for model reporting, display limits, and applying the status separator patch, and the [Pi extension](modules/pi/README.md#herdr-subagent-sidebar) or [Codex hook and watcher](modules/codex/README.md#herdr-subagent-sidebar) for setup and verification.
- Home Manager registers the full [Herdr Annotate plugin](https://github.com/plannotator/herdr-annotate) from `packages/apps/herdr-annotate` in the Nix store. Its plugin source, annotation binary, and document review source and Cargo dependencies are pinned by hash. Nix builds the document reviewer with a local color configuration patch. `Update App Versions` checks every three hours and proposes Herdr, Annotate, and Auto Title updates together in a [candidate PR](packages/apps/README.md#updates-and-ci), independently of other app updates. After the candidate passes CI and is merged, pull the changes and apply Home Manager to install them. Reapplying an older Home Manager generation registers that generation's plugin version while preserving annotation data.
- Herdr builds from Rust and Zig sources with a local patch that passes its resolved palette to plugin panes through `HERDR_PLUGIN_THEME_JSON`. Interactive document reviews inherit `panel_bg` and `text` for the base background and text, plus `selection_bg`, `surface0`, `surface1`, and `accent` for selection, comment highlights, approval highlights, and Markdown code. The snapshot is taken when the pane opens; reopen the review after changing Herdr's theme. After applying Home Manager, restart the Herdr server when its running tasks can be interrupted to load the patched binary.
- Optional `[theme]` values in `modules/herdr/files/plannotator-tui.toml` override inherited colors per field: `background`, `foreground`, `selection_background`, `comment_background`, `looks_good_background`, and `markdown_code_foreground`, each as `#RRGGBB`. Without a Herdr snapshot, omitted fields retain upstream colors; headless commands also retain upstream colors. The environment variable and versioned JSON are a local patch contract, not an upstream API.
...
- In Herdr 0.9.1, document overlays open in the server's active tab; `--deliver-to` selects the feedback recipient. When opening a review from an agent, resolve its pane with `herdr pane current --current`, focus it with `herdr agent focus <pane-id>`, then launch the review. Check the returned `tab_id` against the caller's tab.
- Home Manager also registers [Auto Title](https://github.com/kryptamine/herdr-auto-title) from `packages/apps/herdr-auto-title` in the Nix store. `Update App Versions` tracks stable releases and pins source and Go dependency hashes; Go is needed only during the Nix build. The plugin starts with the Herdr server, so linking or reloading config does not start it. After applying Home Manager, stop the server when its running tasks can be interrupted and reopen Herdr to start the plugin or load its updated binary. Its first run renames existing panes, including manually named panes; later manual renames are preserved.
