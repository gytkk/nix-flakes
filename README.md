...
- Herdr builds from Rust and Zig sources with a local patch that passes its resolved palette to plugin panes through `HERDR_PLUGIN_THEME_JSON`. Interactive document reviews inherit `selection_bg`, `surface0`, `surface1`, and `accent` for selection, comment highlights, approval highlights, and Markdown code. The snapshot is taken when the pane opens; reopen the review after changing Herdr's theme. After applying Home Manager, restart the Herdr server when its running tasks can be interrupted to load the patched binary.
- Optional `[theme]` values in `modules/herdr/files/plannotator-tui.toml` override inherited colors per field: `selection_background`, `comment_background`, `looks_good_background`, and `markdown_code_foreground`, each as `#RRGGBB`. Without a Herdr snapshot, omitted fields retain upstream colors; headless commands also retain upstream colors. The environment variable and versioned JSON are a local patch contract, not an upstream API.
