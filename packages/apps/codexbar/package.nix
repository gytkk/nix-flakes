{
  lib,
  stdenvNoCC,
  fetchurl,
}:

let
  version = "0.56.8";

  platformMap = {
    "aarch64-darwin" = {
      target = "macos-arm64";
      hash = "sha256-MkSCbkK4QJIGjIJ/WpkBcDYMJtHCRzOiuaxV7MsARtI=";
    };

    "x86_64-darwin" = {
      target = "macos-x86_64";
      hash = "sha256-yNubStPuZWj225sh1XYjdtWnSd1JWYtDeiEzO+3b8ns=";
    };

    "x86_64-linux" = {
      target = "linux-musl-x86_64";
      hash = "sha256-/vIZYte2trUgRVySmiEaFi0FqqLU9j7PVKrPvOCoOIc=";
    };

    "aarch64-linux" = {
      target = "linux-musl-aarch64";
      hash = "sha256-J1x0PBOf2vJQhJ5qehT8gQF7/eIzeP6C4/ZOkz/9teM=";
    };
  };

  platform =
    platformMap.${stdenvNoCC.hostPlatform.system}
      or (throw "Unsupported system: ${stdenvNoCC.hostPlatform.system}");

  src = fetchurl {
    url = "https://github.com/steipete/CodexBar/releases/download/v${version}/CodexBarCLI-v${version}-${platform.target}.tar.gz";
    hash = platform.hash;
  };
in
stdenvNoCC.mkDerivation {
  pname = "codexbar";
  inherit version src;

  dontStrip = true;
  sourceRoot = ".";

  installPhase = ''
    runHook preInstall
    mkdir -p $out/bin $out/lib/codexbar
    cp -R ./* $out/lib/codexbar/
    ln -s $out/lib/codexbar/CodexBarCLI $out/bin/codexbar
    runHook postInstall
  '';

  meta = {
    description = "CLI for Codex and Claude usage and cost data";
    homepage = "https://codex.bar";
    license = lib.licenses.mit;
    platforms = builtins.attrNames platformMap;
    mainProgram = "codexbar";
  };
}
